...
if [[ $source == "g" ]]; then
  ...
  read github_url
  git clone $github_url ~/.config/obsidian_config_manager/main_config || (echo -e "${C_RED}Error: invalid github url${NO_FORMAT}" && clean && exit 1)
elif [[ $source == "l" ]]; then
  echo -e "Enter the path to the main_config"
  read path
  mkdir -p ~/.config/obsidian_config_manager/main_config/.obsidian
  cp -r "$path" ~/.config/obsidian_config_manager/main_config/.obsidian/ || (echo -e "${C_RED}Error: invalid path${NO_FORMAT}" && clean && exit 1)
else
